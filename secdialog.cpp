...
{
    QSqlQuery q;
    QVector<double> stat(100);
    stat[0]=0;
    stat[1]=0;
    stat[2]=0;
    stat[3]=0;
    stat[4]=0;
    q.prepare("SELECT  nbplace FROM avionn WHERE nbplace<=50");
    q.exec();
    while (q.next())
    {
        stat[0]++;
    }
    q.prepare("SELECT  nbplace FROM  avionn WHERE nbplace>50 and nbplace<=100");
    q.exec();
    while (q.next())
    {
        stat[1]++;
    }
    q.prepare("SELECT nbplace FROM avionn WHERE nbplace>100 and nbplace<=200");
    q.exec();
    while (q.next())
    {
        stat[2]++;
    }
    q.prepare("SELECT nbplace FROM avionn WHERE nbplace>200 and nbplace<=300");
    q.exec();
    while (q.next())
    {
        stat[3]++;
    }
    q.prepare("SELECT nbplace FROM avionn WHERE nbplace>300");
    q.exec();
    while (q.next())
    {
        stat[4]++;
